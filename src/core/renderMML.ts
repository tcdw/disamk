...
function distortToCommand(note: number): string {
  ...
  return `h${Math.floor(noteDistortMap[note])} $ee $${Math.floor(minor * 256)
    .toString(16)
    .padStart(2, "0")}`;
}

// 改写自 https://github.com/loveemu/spc_converters_legacy/blob/master/nintspc/src/nintspc.c
function getNoteLenForMML(
  tick: number,
  options: {
    division?: number;
    absLen?: boolean;
      ...
      switch (true) {
        ...
        case h === 0xfc: {
          ...
          if (this.state.callID[addr] === null) {
            ...
            this.state.label += 1;
            this.state.rmc.push(
              `(!${(this.state.callID[addr] as number) + 50000})[${this.renderMML({ sequence: this.sequences[addr] })}]`,
            );
          }
          if (e[4] === 0) {
